...
export default function TestEntryForm({ onSubmit, isLoading, defaultDate }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div><label className="text-xs font-semibold text-[var(--text)]/40 uppercase tracking-wide mb-1.5 block">Test Date</label><Input type="date" value={date} onChange={e => setDate(e.target.value)} className="rounded-xl max-w-xs" /></div>
      <div>
        <label className="text-xs font-semibold text-[var(--text)]/40 uppercase tracking-wide mb-3 block">Test Results</label>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {TESTS.map(t => (
            <div key={t.key} className="bg-[var(--bg)] border border-[var(--border)] rounded-xl p-3">
              <div className="flex items-center gap-2 mb-1.5"><span className="text-base">{t.icon}</span><span className="text-sm font-medium text-[var(--text)]/70">{t.label}</span></div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-[var(--text)]/30 whitespace-nowrap">{t.unit}</span>
              </div>
            </div>
          ))}
        </div>
      </div>
      <div><label className="text-xs font-semibold text-[var(--text)]/40 uppercase tracking-wide mb-1.5 block">Notes (optional)</label><Input value={notes} onChange={e => setNotes(e.target.value)} placeholder="Conditions, observations..." className="rounded-xl" /></div>
      <Button type="submit" disabled={!hasValues || isLoading} className="bg-[#1B7A4A] text-[var(--bg)] hover:bg-[#22A05E] rounded-xl px-8">
