    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        {availableMetrics.map(m => (
          <button key={m.key} onClick={() => setSelected(m.key)} className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${selected === m.key ? "text-white shadow-sm" : "bg-[#141414] border border-[#2A2A2A] text-[#F8F8F8]/50 hover:bg-[#2A2A2A]"}`} style={selected === m.key ? { backgroundColor: m.color } : {}}>{m.label}</button>
        ))}
      </div>
        ...
        {chartData.length >= 2 ? (
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={chartData}>
              ...
              <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid #2A2A2A", backgroundColor: "#141414", color: "#F8F8F8", fontSize: 12 }} formatter={v => [`${v} ${metric.unit}`, metric.label]} />
