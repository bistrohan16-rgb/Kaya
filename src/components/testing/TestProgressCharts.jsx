    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        {availableMetrics.map(m => (
          <button key={m.key} onClick={() => setSelected(m.key)} className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${selected === m.key ? "text-white shadow-sm" : "bg-[var(--card)] border border-[var(--border)] text-[var(--text)]/50 hover:bg-[var(--border)]"}`} style={selected === m.key ? { backgroundColor: m.color } : {}}>{m.label}</button>
        ))}
      </div>
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h4 className="font-semibold text-[var(--text)]">{metric.label}</h4>
          ...
          {last != null && <div className="text-right"><p className="text-2xl font-bold text-[var(--text)]">{metric.key.includes("time") || metric.key.includes("seconds") ? formatSeconds(last) : `${last} ${metric.unit}`}</p><p className="text-xs text-[var(--text)]/30">Latest</p></div>}
        </div>
        {chartData.length >= 2 ? (
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={chartData}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
              <XAxis dataKey="date" tick={{ fontSize: 11, fill: "#1B7A4A" }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 11, fill: "#1B7A4A" }} axisLine={false} tickLine={false} width={40} />
              <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid var(--border)", backgroundColor: "var(--card)", color: "var(--text)", fontSize: 12 }} formatter={v => [`${v} ${metric.unit}`, metric.label]} />
        ...
        ) : <div className="h-[180px] flex items-center justify-center"><p className="text-[var(--text)]/30 text-sm">Log at least 2 tests to see your progress chart</p></div>}
      </div>
    </div>
  );
}
