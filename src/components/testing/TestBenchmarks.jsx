...
Provide ratings (excellent/good/average/below_average), top 2 strengths, top 2 areas to improve, and training recommendations.`,
  ...
  const ratingColors = { excellent: "bg-[#1E5C3A]/20 text-emerald-400 border-[#1E5C3A]/30", good: "bg-[#1B7A4A]/10 text-[#1B7A4A] border-[#1B7A4A]/20", average: "bg-[#C44A1A]/10 text-[#C44A1A] border-[#C44A1A]/20", below_average: "bg-red-900/20 text-red-400 border-red-800/30" };
  ...
  return (
    <div className="space-y-4">
      {!analysis && (
        <div className="bg-[var(--card)] border border-[#1B7A4A]/20 rounded-2xl p-5 text-center">
          <Sparkles className="w-8 h-8 text-[#1B7A4A] mx-auto mb-2" />
          <p className="font-semibold text-[var(--text)] mb-1">AI Performance Analysis</p>
          <p className="text-sm text-slate-500 mb-4">Get benchmarked against age and sport norms.</p>
          <Button onClick={analyze} disabled={loading} className="bg-[#1B7A4A] text-[var(--bg)] hover:bg-[#22A05E] rounded-xl">
        ...
        <div className="space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="bg-[#1E5C3A]/10 border border-[#1E5C3A]/20 rounded-2xl p-4">
              ...
              <ul className="space-y-1.5">{analysis.strengths?.map((s,i) => <li key={i} className="text-sm text-[var(--text)]/70 flex gap-2"><span className="text-emerald-500">✓</span>{s}</li>)}</ul>
            </div>
            <div className="bg-[#C44A1A]/10 border border-[#C44A1A]/20 rounded-2xl p-4">
              ...
              <ul className="space-y-1.5">{analysis.improvements?.map((s,i) => <li key={i} className="text-sm text-[var(--text)]/70 flex gap-2"><span className="text-amber-500">→</span>{s}</li>)}</ul>
            </div>
          </div>
          <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-4">
            <p className="text-xs font-bold text-[#1B7A4A] uppercase tracking-wide mb-2">📋 Training Recommendations</p>
            <ul className="space-y-1.5">{analysis.recommendations?.map((r,i) => <li key={i} className="text-sm text-[var(--text)]/70 flex gap-2"><span className="text-blue-500">{i+1}.</span>{r}</li>)}</ul>
          </div>
