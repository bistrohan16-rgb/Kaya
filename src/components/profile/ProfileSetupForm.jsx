...
const StepDot = ({ step, current }) => (
  <div className={`w-2 h-2 rounded-full transition-all ${step <= current ? "bg-[#B8960C]" : "bg-[#2A2A2A]"}`} />
);
...
export default function ProfileSetupForm({ onComplete }) {
  ...
  const TagButton = ({ selected, onClick, children }) => (
    <button onClick={onClick}
      className={`px-3 py-2 rounded-xl text-sm font-body transition-all border ${selected ? "bg-[#B8960C]/10 text-[#B8960C] border-[#B8960C]/30" : "bg-[#141414] text-[#F8F8F8]/40 border-[#2A2A2A] hover:border-[#B8960C]/20 hover:text-[#F8F8F8]/60"}`}>
      {children}
    </button>
  );
          ...
          <div className="space-y-3">
            {[{ mode: "prehab", icon: Shield, title: "Injury Prevention", desc: "I want to prevent injuries and stay at my best." }, { mode: "injury", icon: Activity, title: "Injury Management", desc: "I have a current injury and need a safe programme." }].map(({ mode, icon: Icon, title, desc }) => (
              <button key={mode} onClick={() => { upd("mode", mode); setStep(2); }}
                className={`w-full p-5 rounded-2xl border-2 text-left transition-all hover:border-[#B8960C]/50 ${formData.mode === mode ? "border-[#B8960C]/50 bg-[#B8960C]/5" : "border-[#2A2A2A] bg-[#141414]"}`}>
      ...
      {step === 2 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={() => setStep(formData.mode === "injury" ? 3 : 4)} disabled={!formData.sport || !formData.age || !formData.experience_level} className="flex-1 bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10]">
      ...
      {step === 3 && formData.mode === "injury" && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {INJURY_AREAS.map(area => (
                <button key={area} onClick={() => upd("injured_area", area)}
                  className={`px-3 py-2.5 rounded-xl text-sm font-body transition-all border text-left ${formData.injured_area === area ? "bg-[#B8960C]/10 text-[#B8960C] border-[#B8960C]/30" : "bg-[#141414] text-[#F8F8F8]/40 border-[#2A2A2A] hover:border-[#B8960C]/20"}`}>
                  {area}
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-3 mt-2">
              {[{ v: "mild", label: "Mild", desc: "Some discomfort" }, { v: "moderate", label: "Moderate", desc: "Limits training" }, { v: "severe", label: "Severe", desc: "Cannot train" }].map(({ v, label, desc }) => (
                <button key={v} onClick={() => upd("injury_severity", v)}
                  className={`flex-1 p-3 rounded-xl border text-center transition-all ${formData.injury_severity === v ? "border-[#B8960C]/50 bg-[#B8960C]/5 text-[#B8960C]" : "border-[#2A2A2A] bg-[#141414] text-[#F8F8F8]/40 hover:border-[#B8960C]/20"}`}>
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={() => setStep(4)} disabled={!formData.injured_area || !formData.injury_severity} className="flex-1 bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10]">Continue <ChevronRight className="w-4 h-4 ml-2" /></Button>
          </div>
        </motion.div>
      )}
      ...
      {step === 4 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={handleSubmit} disabled={isLoading || formData.goals.length === 0} className="flex-1 bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10] h-12">
