...
const PAIN_LEVELS = [
  ...
  { value: 1, label: "Very Mild", color: "border-[#B8960C]/30 bg-[#B8960C]/5 text-[#B8960C]" },
  { value: 2, label: "Mild", color: "border-[#B8960C]/50 bg-[#B8960C]/10 text-[#B8960C]" },
...
export default function WorkoutPlayer({ routine, exercises, onComplete, onClose }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 bg-[#0A1628] z-50 flex flex-col">

      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 flex-shrink-0 border-b border-[#2A3F58]/50">
        <div>
          <h2 className="font-display text-lg text-[#F8F8F8]">{routine.name}</h2>
          <p className="text-[#B8960C] text-xs font-body tracking-widest uppercase mt-0.5">
            {phase === "rest" ? "Rest" : phase === "summary" ? "Complete" : phase === "pain_check" ? "Pain Check" : `${currentIdx + 1} of ${exercises.length}`}
          </p>
        </div>
        <button onClick={onClose}
          className="w-9 h-9 rounded-xl bg-[#162232] border border-[#2A3F58] flex items-center justify-center text-[#F8F8F8]/40 hover:text-[#F8F8F8] hover:border-[#B8960C]/30 transition-all">
          <X className="w-4 h-4" />
        </button>
      </div>

      {/* Progress bar */}
      <div className="h-px bg-[#2A3F58] flex-shrink-0">
        <motion.div animate={{ width: `${progress}%` }} className="h-full bg-[#B8960C] transition-all duration-500" />
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "exercise" && currentExercise && (
            <motion.div key={`ex-${currentIdx}`}
              ...
              {/* Exercise name */}
              <h3 className="font-display text-3xl text-[#F8F8F8] text-center mb-1">
                {currentExercise.name}
              </h3>
              {currentExercise.target_areas?.length > 0 && (
                <p className="text-[#B8960C] text-xs font-body tracking-widest uppercase mb-8 text-center">
              ...
              {(currentExercise.duration_seconds || extractDurationFromDescription(currentExercise.description)) ? (
                <div className="relative flex items-center justify-center mb-8">
                  <svg width="196" height="196" className="transform -rotate-90">
                    <circle cx="98" cy="98" r={RING_R} stroke="#2A3F58" strokeWidth="8" fill="none" />
                    <motion.circle cx="98" cy="98" r={RING_R}
                      stroke="#B8960C" strokeWidth="8" fill="none"
                      strokeLinecap="round"
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="font-mono text-6xl font-bold text-[#B8960C] leading-none">
                      {timerVal}
                    </span>
                    <span className="text-[#F8F8F8]/30 text-sm font-body mt-2">seconds</span>
                  </div>
                </div>
              ) : (
                ...
                <div className="flex flex-col items-center mb-8 w-full">
                  ...
                  <div className="flex gap-5 justify-center mb-6">
                    {currentExercise.sets && (
                      <div className="bg-[#162232] border border-[#2A3F58] rounded-2xl px-8 py-5 text-center min-w-[100px]">
                        <p className="font-mono text-5xl text-[#F8F8F8] leading-none">{currentExercise.sets}</p>
                        <p className="text-[#F8F8F8]/30 text-sm font-body mt-2">sets</p>
                      </div>
                    )}
                    {currentExercise.reps && (
                      <div className="bg-[#162232] border border-[#B8960C]/30 rounded-2xl px-8 py-5 text-center min-w-[100px]">
                        <p className="font-mono text-5xl text-[#B8960C] leading-none">{currentExercise.reps}</p>
                        <p className="text-[#F8F8F8]/30 text-sm font-body mt-2">reps</p>
                      </div>
                    )}
                    {!currentExercise.sets && !currentExercise.reps && (
                      <div className="bg-[#162232] border border-[#2A3F58] rounded-2xl px-8 py-5 text-center">
                        <p className="font-mono text-3xl text-[#B8960C] leading-none">Go</p>
                        <p className="text-[#F8F8F8]/30 text-sm font-body mt-2">at your pace</p>
                      </div>
                    )}
                  </div>
                  {/* Elapsed stopwatch so user has something to watch */}
                  <div className="flex items-center gap-2 text-[#F8F8F8]/20">
              ...
              {currentExercise.description && (
                <div className="w-full bg-[#162232] border border-[#2A3F58] rounded-2xl p-5 mb-8">
                  <p className="text-[#B8960C] text-xs font-body font-semibold tracking-widest uppercase mb-3">
                    How to perform
                  </p>
                  <p className="text-[#F8F8F8]/70 text-sm font-body leading-relaxed">
                    {currentExercise.description}
                  </p>
                </div>
              )}
              ...
              <Button onClick={handleDoneExercise}
                className="w-full h-14 bg-[#B8960C] text-[#0A1628] hover:bg-[#D4AA10] font-semibold text-base rounded-2xl">
                Done — Next Exercise
              </Button>

              <p className="text-[#F8F8F8]/15 text-[10px] font-body text-center mt-4">
          ...
          {phase === "pain_check" && (
            <motion.div key="pain"
              ...
              <h3 className="font-display text-2xl text-[#F8F8F8] text-center mb-1">How did that feel?</h3>
              <p className="text-[#F8F8F8]/40 text-sm font-body text-center mb-8">
                Rate any pain during <span className="text-[#F8F8F8]/70">{currentExercise?.name}</span>
              </p>
              <div className="grid grid-cols-3 gap-2 w-full mb-6">
                {PAIN_LEVELS.map(p => (
                  <button key={p.value} onClick={() => setPainLevel(p.value)}
                    className={`p-3 rounded-xl border-2 transition-all ${painLevel === p.value ? p.color : "border-[#2A3F58] bg-[#162232] text-[#F8F8F8]/30"}`}>
              ...
              <Button onClick={handlePainConfirmed}
                className="w-full h-12 bg-[#B8960C] text-[#0A1628] hover:bg-[#D4AA10] font-semibold rounded-2xl">
            ...
            <motion.div key="rest"
              ...
              <div className="relative flex items-center justify-center mb-8">
                <svg width="210" height="210" className="transform -rotate-90">
                  <circle cx="105" cy="105" r={REST_R} stroke="#2A3F58" strokeWidth="8" fill="none" />
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="font-mono text-6xl font-bold text-[#B8960C] leading-none">{restTime}</span>
                  <span className="text-[#F8F8F8]/30 text-sm font-body mt-2">seconds</span>
                </div>
              </div>
              {exercises[currentIdx + 1] && (
                <div className="text-center mb-8">
                  <p className="text-[#F8F8F8]/30 text-xs font-body uppercase tracking-widest mb-1">Next up</p>
                  <p className="font-display text-xl text-[#F8F8F8]">{exercises[currentIdx + 1].name}</p>
                </div>
              )}
              <button onClick={() => { clearInterval(restTimerRef.current); advanceToNext(); }}
                className="text-[#B8960C]/40 text-sm font-body hover:text-[#B8960C] transition-colors underline">
                Skip rest
              </button>
            </motion.div>
          )}
          ...
          {phase === "summary" && (
            <motion.div key="summary"
              ...
              <div className="w-16 h-16 bg-[#B8960C]/10 border border-[#B8960C]/20 rounded-2xl flex items-center justify-center mx-auto mb-5">
                <Check className="w-8 h-8 text-[#B8960C]" />
              </div>
              <h3 className="font-display text-3xl text-[#F8F8F8] mb-1">Session complete.</h3>
              <p className="text-[#F8F8F8]/30 text-sm font-body mb-8">{formatTime(elapsed)} · {exercises.length} exercises</p>
              <div className="bg-[#162232] border border-[#2A3F58] rounded-2xl p-4 mb-6 w-full space-y-2">
                {exerciseLogs.map((log, i) => (
                  <div key={i} className="flex items-center justify-between text-sm">
                    <span className="text-[#F8F8F8]/60 font-body truncate mr-3">{log.exercise_name}</span>
                    <span className={`text-xs font-mono px-2 py-0.5 rounded-full flex-shrink-0 ${
                      ...
                      log.pain_level <= 2 ? "bg-[#B8960C]/10 text-[#B8960C]" :
                  ...
                  <button key={f.value} onClick={() => setOverallFeeling(f.value)}
                    className={`px-4 py-2 rounded-xl text-sm font-body transition-all border ${
                      overallFeeling === f.value
                        ? "bg-[#B8960C] text-[#0A1628] border-[#B8960C]"
                        : "border-[#2A3F58] text-[#F8F8F8]/40 hover:border-[#B8960C]/30"
                    }`}>
                    {f.label}
                  </button>
                ))}
      ...
      {/* Bottom controls */}
      {phase === "exercise" && (
        <div className="flex items-center justify-center gap-6 px-5 py-4 flex-shrink-0 border-t border-[#2A3F58]/30">
          <button onClick={() => { if (currentIdx > 0) { setCurrentIdx(p => p - 1); setPhase("exercise"); setPainLevel(0); } }}
            disabled={currentIdx === 0}
            className="w-12 h-12 rounded-xl bg-[#162232] border border-[#2A3F58] flex items-center justify-center text-[#F8F8F8]/30 disabled:opacity-20 hover:border-[#B8960C]/30 hover:text-[#F8F8F8]/60 transition-all">
            <SkipBack className="w-5 h-5" />
          </button>
          <button onClick={() => setIsPaused(v => !v)}
            className="w-12 h-12 rounded-xl bg-[#162232] border border-[#2A3F58] flex items-center justify-center text-[#F8F8F8]/40 hover:border-[#B8960C]/30 hover:text-[#F8F8F8]/70 transition-all">
            {isPaused ? <Play className="w-5 h-5" /> : <Pause className="w-5 h-5" />}
          </button>
          <button onClick={handleDoneExercise}
            className="w-12 h-12 rounded-xl bg-[#162232] border border-[#2A3F58] flex items-center justify-center text-[#F8F8F8]/30 hover:border-[#B8960C]/30 hover:text-[#F8F8F8]/60 transition-all">
