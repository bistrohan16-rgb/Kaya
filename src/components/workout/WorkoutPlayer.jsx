...
export default function WorkoutPlayer({ routine, exercises, onComplete, onClose }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 bg-[#080808] z-50 flex flex-col">

      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 flex-shrink-0 border-b border-[#2A2A2A]/50">
        <div>
        ...
        <button onClick={onClose}
          className="w-9 h-9 rounded-xl bg-[#141414] border border-[#2A2A2A] flex items-center justify-center text-[#F8F8F8]/40 hover:text-[#F8F8F8] hover:border-[#B8960C]/30 transition-all">
          <X className="w-4 h-4" />
        </button>
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "exercise" && currentExercise && (
            <motion.div key={`ex-${currentIdx}`}
              ...
              {(currentExercise.duration_seconds || extractDurationFromDescription(currentExercise.description)) ? (
                <div className="relative flex items-center justify-center mb-8">
                  <svg width="196" height="196" className="transform -rotate-90">
                    <circle cx="98" cy="98" r={RING_R} stroke="#2A2A2A" strokeWidth="8" fill="none" />
                ...
                <div className="flex flex-col items-center mb-8 w-full">
                  ...
                  <div className="flex gap-5 justify-center mb-6">
                    {currentExercise.sets && (
                      <div className="bg-[#141414] border border-[#2A2A2A] rounded-2xl px-8 py-5 text-center min-w-[100px]">
                    ...
                    {currentExercise.reps && (
                      <div className="bg-[#141414] border border-[#B8960C]/30 rounded-2xl px-8 py-5 text-center min-w-[100px]">
                    ...
                    {!currentExercise.sets && !currentExercise.reps && (
                      <div className="bg-[#141414] border border-[#2A2A2A] rounded-2xl px-8 py-5 text-center">
              ...
              {currentExercise.description && (
                <div className="w-full bg-[#141414] border border-[#2A2A2A] rounded-2xl p-5 mb-8">
              ...
              <Button onClick={handleDoneExercise}
                className="w-full h-14 bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10] font-semibold text-base rounded-2xl">
                Done — Next Exercise
              </Button>
          ...
          {phase === "pain_check" && (
            <motion.div key="pain"
              ...
              <div className="grid grid-cols-3 gap-2 w-full mb-6">
                {PAIN_LEVELS.map(p => (
                  <button key={p.value} onClick={() => setPainLevel(p.value)}
                    className={`p-3 rounded-xl border-2 transition-all ${painLevel === p.value ? p.color : "border-[#2A2A2A] bg-[#141414] text-[#F8F8F8]/30"}`}>
              ...
              <Button onClick={handlePainConfirmed}
                className="w-full h-12 bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10] font-semibold rounded-2xl">
            ...
            <motion.div key="rest"
              ...
              <div className="relative flex items-center justify-center mb-8">
                <svg width="210" height="210" className="transform -rotate-90">
                  <circle cx="105" cy="105" r={REST_R} stroke="#2A2A2A" strokeWidth="8" fill="none" />
          ...
          {phase === "summary" && (
            <motion.div key="summary"
              ...
              <div className="bg-[#141414] border border-[#2A2A2A] rounded-2xl p-4 mb-6 w-full space-y-2">
                  ...
                  <button key={f.value} onClick={() => setOverallFeeling(f.value)}
                    className={`px-4 py-2 rounded-xl text-sm font-body transition-all border ${
                      overallFeeling === f.value
                        ? "bg-[#B8960C] text-[#080808] border-[#B8960C]"
                        : "border-[#2A2A2A] text-[#F8F8F8]/40 hover:border-[#B8960C]/30"
                    }`}>
                    {f.label}
                  </button>
                ))}
      ...
      {/* Bottom controls */}
      {phase === "exercise" && (
        <div className="flex items-center justify-center gap-6 px-5 py-4 flex-shrink-0 border-t border-[#2A2A2A]/30">
          <button onClick={() => { if (currentIdx > 0) { setCurrentIdx(p => p - 1); setPhase("exercise"); setPainLevel(0); } }}
            disabled={currentIdx === 0}
            className="w-12 h-12 rounded-xl bg-[#141414] border border-[#2A2A2A] flex items-center justify-center text-[#F8F8F8]/30 disabled:opacity-20 hover:border-[#B8960C]/30 hover:text-[#F8F8F8]/60 transition-all">
            <SkipBack className="w-5 h-5" />
          </button>
          <button onClick={() => setIsPaused(v => !v)}
            className="w-12 h-12 rounded-xl bg-[#141414] border border-[#2A2A2A] flex items-center justify-center text-[#F8F8F8]/40 hover:border-[#B8960C]/30 hover:text-[#F8F8F8]/70 transition-all">
            {isPaused ? <Play className="w-5 h-5" /> : <Pause className="w-5 h-5" />}
          </button>
          <button onClick={handleDoneExercise}
            className="w-12 h-12 rounded-xl bg-[#141414] border border-[#2A2A2A] flex items-center justify-center text-[#F8F8F8]/30 hover:border-[#B8960C]/30 hover:text-[#F8F8F8]/60 transition-all">
