            ...
            <motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              {ALL_SPORTS.map((sport, i) => (
                <motion.button key={sport} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  className="bg-[#141414] border border-[#2A2A2A] rounded-2xl p-5 text-left hover:border-[#B8960C]/40 hover:bg-[#1C1C1C] transition-all group">
                  ...
                  <ChevronRight className="w-4 h-4 text-[#2A2A2A] group-hover:text-[#B8960C] mt-3 transition-colors" />
                </motion.button>
              ))}
            </motion.div>
          ) : (
            <motion.div key="plans" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
              {sportData?.plans.map((plan, i) => (
                <motion.div key={plan.name} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                  className="bg-[#141414] border border-[#2A2A2A] rounded-2xl overflow-hidden hover:border-[#B8960C]/20 transition-all">
                  <div className="p-5 flex items-start justify-between">
                    ...
                    <button onClick={() => setExpandedPlan(expandedPlan === plan.name ? null : plan.name)}
                      className="text-[#2A2A2A] hover:text-[#B8960C] transition-colors mt-1">
                      <ChevronRight className={`w-5 h-5 transition-transform ${expandedPlan === plan.name ? "rotate-90" : ""}`} />
                    </button>
                  </div>
                  <AnimatePresence>
                    {expandedPlan === plan.name && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden border-t border-[#2A2A2A]/50">
                        <div className="p-5 space-y-3">
                          {plan.exercises.map((ex, j) => (
                            <div key={j} className="flex items-center gap-3 bg-[#080808] rounded-xl p-3 border border-[#2A2A2A]/50">
