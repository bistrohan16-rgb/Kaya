...
export default function Testing() {
  ...
  return (
    <div className="min-h-screen bg-[#080808]">
      {/* Header */}
      <div className="bg-[#0E0E0E] border-b border-[#2A2A2A]/50 px-5 pt-8 pb-6">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
        <div className="flex justify-end">
          <Button onClick={() => setShowForm(v => !v)} variant={showForm ? "ghost" : "default"}
            className={showForm ? "text-[#F8F8F8]/40" : "bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10]"}>
        ...
        {tests.length === 0 && !isLoading && (
          <div className="bg-[#141414] border border-[#2A2A2A] rounded-2xl p-16 text-center">
            <FlaskConical className="w-12 h-12 text-[#2A2A2A] mx-auto mb-4" />
            ...
            <Button onClick={() => setShowForm(true)} className="bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10]">
        ...
        {tests.length > 0 && (
          <>
            {/* Tabs */}
            <div className="flex gap-1 bg-[#141414] border border-[#2A2A2A] rounded-2xl p-1">
              {[{ id: "history", label: "History" }, { id: "charts", label: "Progress Charts" }, { id: "analysis", label: "AI Analysis" }].map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex-1 py-2.5 px-4 rounded-xl text-sm font-body font-medium transition-all ${activeTab === tab.id ? "bg-[#B8960C] text-[#080808]" : "text-[#F8F8F8]/40 hover:text-[#F8F8F8]/70"}`}>
                  {tab.label}
                </button>
              ))}
            </div>
                    ...
                    <motion.div key={test.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                      ...
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <button onClick={() => window.confirm("Delete this test?") && deleteMutation.mutate(test.id)}
                          className="w-8 h-8 rounded-lg flex items-center justify-center text-[#2A2A2A] hover:text-red-400 hover:bg-red-900/20 transition-all">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {keys.map(k => (
                          <span key={k} className="inline-flex items-center gap-2 bg-[#080808] border border-[#2A2A2A] text-xs font-mono text-[#B8960C] px-3 py-1.5 rounded-xl">
