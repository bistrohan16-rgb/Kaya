...
export default function Testing() {
  ...
  return (
    <div className="min-h-screen bg-[#0A1628]">
      {/* Header */}
      <div className="bg-[#0F2035] border-b border-[#2A3F58]/50 px-5 pt-8 pb-6">
        <div className="max-w-5xl mx-auto">
          <p className="kaya-label mb-2">Performance</p>
          <h1 className="font-display text-3xl text-[#F8F8F8]">Testing Tracker</h1>
          <p className="text-[#F8F8F8]/40 text-sm font-body mt-1">Log monthly tests, track progress and benchmark against sport norms.</p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
        <div className="flex justify-end">
          <Button onClick={() => setShowForm(v => !v)} variant={showForm ? "ghost" : "default"}
            className={showForm ? "text-[#F8F8F8]/40" : "bg-[#B8960C] text-[#0A1628] hover:bg-[#D4AA10]"}>
        ...
        <AnimatePresence>
          {showForm && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
              <div className="bg-[#162232] border border-[#2A3F58] rounded-2xl p-6">
                <h3 className="font-display text-xl text-[#F8F8F8] mb-5">New Test Session</h3>
        ...
        {tests.length === 0 && !isLoading && (
          <div className="bg-[#162232] border border-[#2A3F58] rounded-2xl p-16 text-center">
            <FlaskConical className="w-12 h-12 text-[#2A3F58] mx-auto mb-4" />
            <p className="font-display text-xl text-[#F8F8F8] mb-2">No test results yet</p>
            <p className="text-[#F8F8F8]/40 text-sm font-body mb-6">Log your first session to start tracking performance.</p>
            <Button onClick={() => setShowForm(true)} className="bg-[#B8960C] text-[#0A1628] hover:bg-[#D4AA10]">
        ...
        {tests.length > 0 && (
          <>
            {/* Tabs */}
            <div className="flex gap-1 bg-[#162232] border border-[#2A3F58] rounded-2xl p-1">
              {[{ id: "history", label: "History" }, { id: "charts", label: "Progress Charts" }, { id: "analysis", label: "AI Analysis" }].map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex-1 py-2.5 px-4 rounded-xl text-sm font-body font-medium transition-all ${activeTab === tab.id ? "bg-[#B8960C] text-[#0A1628]" : "text-[#F8F8F8]/40 hover:text-[#F8F8F8]/70"}`}>
                  {tab.label}
                </button>
              ))}
            </div>
                ...
                {sortedTests.map((test, i) => {
                  ...
                  return (
                    <motion.div key={test.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                      className="bg-[#162232] border border-[#2A3F58] rounded-2xl p-5 hover:border-[#B8960C]/20 transition-all">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <p className="font-display text-lg text-[#F8F8F8]">{format(new Date(test.test_date), "d MMMM yyyy")}</p>
                        ...
                        <button onClick={() => window.confirm("Delete this test?") && deleteMutation.mutate(test.id)}
                          className="w-8 h-8 rounded-lg flex items-center justify-center text-[#2A3F58] hover:text-red-400 hover:bg-red-900/20 transition-all">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {keys.map(k => (
                          <span key={k} className="inline-flex items-center gap-2 bg-[#0A1628] border border-[#2A3F58] text-xs font-mono text-[#B8960C] px-3 py-1.5 rounded-xl">
            ...
            {activeTab === "charts" && (
              <div className="bg-[#162232] border border-[#2A3F58] rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-5">
                  <BarChart3 className="w-5 h-5 text-[#B8960C]" />
                  <h3 className="font-display text-xl text-[#F8F8F8]">Progress Over Time</h3>
                </div>
