          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {TYPES.map(t => (
              <button key={t.value} onClick={() => setSelectedType(t.value)}
                className={`px-4 py-1.5 rounded-full text-xs font-body font-semibold whitespace-nowrap flex-shrink-0 transition-all ${selectedType === t.value ? "bg-[#B8960C] text-[#080808]" : "bg-[#141414] border border-[#2A2A2A] text-[#F8F8F8]/50 hover:border-[#B8960C]/30"}`}>
                {t.label}
              </button>
            ))}
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <BookOpen className="w-12 h-12 text-[#2A2A2A] mx-auto mb-4" />
              ...
              {filtered.map((routine, i) => (
                <motion.div key={routine.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <Button onClick={() => startRoutine(routine)} variant="outline" className="flex-shrink-0 group-hover:bg-[#B8960C] group-hover:text-[#080808] group-hover:border-[#B8960C] transition-all">
