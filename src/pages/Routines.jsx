  ...
  return (
    <div className="min-h-screen bg-[#0A1628]">
      <div className="bg-[#0F2035] border-b border-[#2A3F58]/50 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="font-display text-2xl text-[#F8F8F8]">My Routines</h1>
              <p className="text-[#B8960C] text-xs font-body tracking-widest uppercase">{routines.length} programmes</p>
            </div>
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {TYPES.map(t => (
              <button key={t.value} onClick={() => setSelectedType(t.value)}
                className={`px-4 py-1.5 rounded-full text-xs font-body font-semibold whitespace-nowrap flex-shrink-0 transition-all ${selectedType === t.value ? "bg-[#B8960C] text-[#0A1628]" : "bg-[#162232] border border-[#2A3F58] text-[#F8F8F8]/50 hover:border-[#B8960C]/30"}`}>
                {t.label}
              </button>
            ))}
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        {isLoading ? (
          <div className="flex justify-center py-20"><Loader2 className="w-6 h-6 animate-spin text-[#B8960C]" /></div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-20">
            <BookOpen className="w-12 h-12 text-[#2A3F58] mx-auto mb-4" />
            <p className="text-[#F8F8F8]/30 font-body mb-4">No routines in this category</p>
            ...
            <AnimatePresence>
              {filtered.map((routine, i) => (
                <motion.div key={routine.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  className="bg-[#162232] border border-[#2A3F58] rounded-2xl p-5 hover:border-[#B8960C]/30 transition-all group">
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        <Badge variant={TYPE_BADGE[routine.type] || "default"} className="capitalize">{routine.type?.replace("_", " ")}</Badge>
                        {routine.difficulty && <span className="text-xs text-[#F8F8F8]/30 font-body capitalize">{routine.difficulty}</span>}
                      </div>
                      <h3 className="font-display text-lg text-[#F8F8F8] group-hover:text-[#B8960C] transition-colors">{routine.name}</h3>
                      {routine.description && <p className="text-[#F8F8F8]/40 text-sm font-body mt-1 line-clamp-1">{routine.description}</p>}
                      <div className="flex items-center gap-4 mt-3 text-xs font-mono text-[#B8960C]/60">
                    ...
                    <Button onClick={() => startRoutine(routine)} variant="outline" className="flex-shrink-0 group-hover:bg-[#B8960C] group-hover:text-[#0A1628] group-hover:border-[#B8960C] transition-all">
