...
export default function Login() {
  ...
  if (mode === "login" && initialMode === "signup") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === "login") {
        ...
        // Coach signup goes to onboarding
        if (plan === "coach") {
          navigate("/CoachOnboarding");
        } else {
          navigate(redirect);
        }
      }
    ...
    <div className="min-h-screen bg-[#080808] flex flex-col items-center justify-center px-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm relative z-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <Button type="submit" disabled={isLoading} className="w-full h-12 bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10] font-semibold">
