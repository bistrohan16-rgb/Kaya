...
export default function Dashboard() {
  ...
  if (profileLoading) return <div className="min-h-screen bg-[#0A1628] flex items-center justify-center"><KayaMark size={48} pulse /></div>;
  ...
  if (!profile) return (
    <div className="min-h-screen bg-[#0A1628] py-12 px-4">
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-[#162232] border border-[#2A3F58] rounded-2xl p-8"><ProfileSetupForm onComplete={handleProfileComplete} /></div>
      </div>
    </div>
  );
      ...
      <div className="space-y-2 w-full max-w-xs">
        {["Analysing your profile...", "Generating exercises...", "Building routines...", "Saving to your account..."].map((s, i) => (
          <motion.div key={s} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.5 }}
            className="flex items-center gap-3 bg-[#162232] border border-[#2A3F58] rounded-xl px-4 py-3">
  ...
  return (
    <div className="min-h-screen bg-[#0A1628]">
      {/* Header */}
      <div className="bg-[#0F2035] border-b border-[#2A3F58]/50 px-5 pt-6 pb-5">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-3">
          {[{ icon: Calendar, label: "This week", value: thisWeekLogs.length, unit: "sessions" }, { icon: Clock, label: "Total time", value: totalMinutes, unit: "minutes" }, { icon: Target, label: "Streak", value: Math.min(thisWeekLogs.length, 7), unit: "days" }, { icon: TrendingUp, label: "All time", value: workoutLogs.length, unit: "sessions" }].map(s => (
            <div key={s.label} className="bg-[#162232] border border-[#2A3F58] rounded-2xl p-4">
        ...
        {/* Quick start */}
        {routines.length > 0 && (
          <div className="bg-[#162232] border border-[#B8960C]/20 rounded-2xl p-5 flex items-center justify-between">
            <div>
            ...
            <Button onClick={() => { const r = routines.find(r => r.type === "warmup" || r.type === "prehab") || routines[0]; startRoutine(r); }} className="bg-[#B8960C] text-[#0A1628] hover:bg-[#D4AA10] flex-shrink-0">
        ...
        <div>
          ...
          {routines.length === 0 ? (
            <div className="bg-[#162232] border border-[#2A3F58] rounded-2xl p-10 text-center">
              ...
              <Button onClick={() => generateContent(profile)} className="bg-[#B8960C] text-[#0A1628] hover:bg-[#D4AA10]"><Sparkles className="w-4 h-4 mr-2" />Generate Programme</Button>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 gap-3">
              {routines.slice(0, 4).map((routine, i) => (
                <motion.div key={routine.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  className="bg-[#162232] border border-[#2A3F58] rounded-2xl p-4 hover:border-[#B8960C]/30 transition-all group cursor-pointer" onClick={() => startRoutine(routine)}>
                  <div className="flex items-start justify-between">
                    ...
                    <ChevronRight className="w-4 h-4 text-[#2A3F58] group-hover:text-[#B8960C] flex-shrink-0 mt-1 transition-colors" />
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[{ label: "Exercise Library", icon: Dumbbell, path: "Exercises" }, { label: "Sport Plans", icon: BookOpen, path: "SportPlans" }, { label: "Performance Testing", icon: FlaskConical, path: "Testing" }].map(({ label, icon: Icon, path }) => (
            <Link key={path} to={createPageUrl(path)}>
              <div className="bg-[#162232] border border-[#2A3F58] rounded-2xl p-4 hover:border-[#B8960C]/30 transition-all group text-center">
