  ...
  return (
    <div className="relative flex items-center justify-center">
      <svg width="90" height="90" className="transform -rotate-90">
        <circle cx="45" cy="45" r={r} stroke="#2A2A2A" strokeWidth="5" fill="none" />
        <circle cx="45" cy="45" r={r} stroke="#B8960C" strokeWidth="5" fill="none"
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="font-mono text-xl font-bold text-[#B8960C] leading-none">{sessions}</span>
        <span className="text-[#F8F8F8]/30 text-[9px] font-body mt-0.5">/ {target}</span>
      </div>
    </div>
  );
}
...
function PainSparkline({ logs }) {
  ...
  const color = maxPain >= 4 ? "#ef4444" : maxPain >= 3 ? "#C44A1A" : "#B8960C";
  return (
    <div className="flex items-center gap-3">
      <div>
        <p className="text-[#F8F8F8]/30 text-[9px] font-body uppercase tracking-widest mb-1">Pain trend</p>
      ...
      <div className={`text-xs font-mono px-2 py-1 rounded-lg ${maxPain === 0 ? "bg-emerald-900/30 text-emerald-400" : maxPain <= 2 ? "bg-[#B8960C]/10 text-[#B8960C]" : "bg-red-900/20 text-red-400"}`}>
        Peak {maxPain}/5
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#080808]">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-3">
          {/* Weekly ring */}
          <div className="bg-[#141414] border border-[#2A2A2A] rounded-2xl p-4 flex flex-col items-center justify-center">
            <WeeklyRing sessions={thisWeekLogs.length} target={profile?.training_days_per_week || 4} />
            <p className="text-[9px] font-body text-[#F8F8F8]/30 mt-2 tracking-widest uppercase">This week</p>
          </div>
          {[
