    ...
    <div className="bg-[#141414] border border-[#2A2A2A] rounded-2xl overflow-hidden w-full">
      <div className="bg-[#0E0E0E] border-b border-[#2A2A2A] px-4 py-3 flex items-center justify-between">
        <div>
          ...
          <p className="text-[#B8960C] text-[10px] font-body tracking-widest uppercase">4 active athletes</p>
        </div>
      ...
      {/* Stats */}
      <div className="grid grid-cols-3 gap-2 p-3 border-b border-[#2A2A2A]">
      ...
      <div className="p-3 space-y-1.5">
        {athletes.map((a, i) => (
          <div key={i} className="bg-[#0E0E0E] border border-[#2A2A2A] rounded-xl px-3 py-2 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full flex-shrink-0 ${a.status === "green" ? "bg-emerald-400" : "bg-amber-400"}`} />
              <div>
                <p className="text-[#F8F8F8] text-xs font-body">{a.name}</p>
                <p className="text-[#F8F8F8]/30 text-[8px] font-body">{a.sport} · {a.sessions} sessions</p>
              </div>
            </div>
            <span className={`text-[8px] font-mono px-1.5 py-0.5 rounded-full ${a.pain === 0 ? "bg-emerald-900/30 text-emerald-400" : a.pain <= 2 ? "bg-[#B8960C]/10 text-[#B8960C]" : "bg-amber-900/30 text-amber-400"}`}>
              Pain: {a.pain}/5
            </span>
          </div>
        ))}
      ...
      <div className="relative z-10">
        ...
        <motion.nav initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/login?mode=signup&plan=coach">
              <Button className="bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10] rounded-xl font-semibold">
                Start Free Trial
              </Button>
            </Link>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                ...
                <span className="text-[#B8960C]">Before they break.</span>
              </motion.h1>

              <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                className="text-[#F8F8F8]/50 text-base font-body leading-relaxed mb-6">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
                className="bg-[#141414] border border-[#B8960C]/20 rounded-2xl p-5 mb-6">
                <div className="flex items-center gap-2 mb-2">
                  <Clock className="w-4 h-4 text-[#B8960C]" />
                  <p className="text-[#B8960C] text-xs font-body font-semibold tracking-widest uppercase">7-Day Free Trial</p>
                </div>
                <p className="text-[#F8F8F8]/60 text-sm font-body mb-3">
                  Get full access to the Coach plan free for 7 days. After your trial, Kaya Coach is <strong className="text-[#F8F8F8]">£24.99/month</strong>. Cancel anytime before your trial ends and you won't be charged.
                </p>
                <Link to="/login?mode=signup&plan=coach">
                  <Button className="w-full h-11 bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10] font-semibold rounded-xl">
                    Start Your Free Trial <ChevronRight className="w-4 h-4 ml-1" />
                  </Button>
                </Link>
                <p className="text-[#F8F8F8]/25 text-[10px] font-body text-center mt-2">No credit card required to start · £24.99/month after trial</p>
              </motion.div>
        ...
        <div className="max-w-6xl mx-auto px-5 mb-24">
          <div className="text-center mb-12">
            ...
            <h2 className="font-display text-4xl text-[#F8F8F8]">The complete platform<br />for athlete welfare.</h2>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-5 mb-24">
          <div className="text-center mb-12">
            ...
            <h2 className="font-display text-4xl text-[#F8F8F8]">Up and running<br />in minutes.</h2>
          </div>
          <div className="grid md:grid-cols-4 gap-5">
            {STEPS.map((step, i) => (
              <motion.div key={step.step} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                className="bg-[#141414] border border-[#2A2A2A] rounded-2xl p-5 hover:border-[#B8960C]/20 transition-all">
                <p className="font-mono text-[#B8960C]/40 text-xs mb-3 tracking-widest">{step.step}</p>
                <h3 className="font-display text-lg text-[#F8F8F8] mb-2">{step.title}</h3>
                <p className="text-[#F8F8F8]/40 text-xs font-body leading-relaxed">{step.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto px-5 mb-24">
          <div className="bg-[#141414] border border-[#B8960C]/20 rounded-2xl p-8 text-center">
            <p className="kaya-label mb-4">Simple pricing</p>
            <div className="flex items-baseline justify-center gap-2 mb-2">
              <span className="font-mono text-5xl text-[#B8960C]">£24.99</span>
              <span className="text-[#F8F8F8]/30 font-body">/month</span>
            </div>
            <p className="text-[#F8F8F8]/40 font-body text-sm mb-6">after your 7-day free trial · cancel anytime</p>
            <div className="grid grid-cols-2 gap-3 mb-8 text-left">
              {["Unlimited athletes", "Pain alert system", "Custom routine builder", "Team selection report", "Performance testing", "Compliance tracking", "AI Coach access", "Priority support"].map(f => (
                <div key={f} className="flex items-center gap-2">
                  <Check className="w-3.5 h-3.5 text-[#B8960C] flex-shrink-0" />
                  <span className="text-[#F8F8F8]/60 text-xs font-body">{f}</span>
                </div>
              ))}
            </div>
            <Link to="/login?mode=signup&plan=coach">
              <Button className="w-full h-12 bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10] font-semibold rounded-xl">
                Start 7-Day Free Trial <ChevronRight className="w-4 h-4 ml-1" />
              </Button>
            </Link>
            <p className="text-[#F8F8F8]/20 text-[10px] font-body mt-3">No credit card required · Full access during trial · Cancel before day 7 to avoid charge</p>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-5 mb-20 text-center">
          ...
          <h2 className="font-display text-3xl text-[#F8F8F8] mb-3">Already an athlete on Kaya?</h2>
          <p className="text-[#F8F8F8]/40 font-body mb-6 text-sm">Head back to the main app to start your prehab programme.</p>
          <Link to="/">
            <Button variant="outline" className="border-[#2A2A2A] text-[#F8F8F8]/40 hover:border-[#B8960C]/40 hover:text-[#B8960C]">
              Back to Kaya
            </Button>
          </Link>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-5 pb-12 text-center">
          <p className="text-[#F8F8F8]/15 text-xs font-body leading-relaxed max-w-2xl mx-auto">
            ⚕️ <strong className="text-[#F8F8F8]/25">Medical Disclaimer:</strong> Kaya is a fitness companion app, not a medical product. Not a replacement for physiotherapy or professional healthcare advice.
          </p>
        </div>
      </div>
    </div>
