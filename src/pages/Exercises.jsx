        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          ...
          <div className="relative mb-3">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search exercises, muscles..."
              className="pl-10 bg-[#162232] border-[#2A3F58]" />
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {TYPES.map(t => (
              <button key={t.value} onClick={() => setSelectedType(t.value)}
                className={`px-4 py-1.5 rounded-full text-xs font-body font-semibold whitespace-nowrap transition-all flex-shrink-0 ${selectedType === t.value ? "bg-[#B8960C] text-[#0A1628]" : "bg-[#162232] text-[#F8F8F8]/50 border border-[#2A3F58] hover:border-[#B8960C]/30 hover:text-[#F8F8F8]"}`}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {filtered.map((ex, i) => (
              <motion.div key={ex.name} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.02, 0.4) }}
                ...
                className="bg-[#162232] border border-[#2A3F58] rounded-2xl p-4 hover:border-[#B8960C]/30 hover:bg-[#1E2E42] transition-all cursor-pointer group">
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                  {ex.target_areas?.slice(0, 2).map(area => (
                    <span key={area} className="text-[10px] text-[#F8F8F8]/30 bg-[#2A3F58]/50 px-2 py-0.5 rounded-full font-body">{area}</span>
                  ))}
                </div>
              ...
              <div className="space-y-5">
                {selectedExercise.target_areas?.length > 0 && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">{selectedExercise.target_areas.map(a => <span key={a} className="px-3 py-1.5 bg-[#2A3F58] text-[#F8F8F8]/70 rounded-lg text-sm font-body">{a}</span>)}</div>
                  </div>
                )}
                <div className="grid grid-cols-3 gap-3">
                  {selectedExercise.duration_seconds && <div className="bg-[#162232] border border-[#2A3F58] rounded-xl p-4 text-center"><p className="text-2xl font-mono text-[#B8960C]">{selectedExercise.duration_seconds}s</p><p className="text-xs text-[#F8F8F8]/30 mt-1 font-body">Duration</p></div>}
                  {selectedExercise.reps && <div className="bg-[#162232] border border-[#2A3F58] rounded-xl p-4 text-center"><p className="text-2xl font-mono text-[#B8960C]">{selectedExercise.reps}</p><p className="text-xs text-[#F8F8F8]/30 mt-1 font-body">Reps</p></div>}
                  {selectedExercise.sets && <div className="bg-[#162232] border border-[#2A3F58] rounded-xl p-4 text-center"><p className="text-2xl font-mono text-[#B8960C]">{selectedExercise.sets}</p><p className="text-xs text-[#F8F8F8]/30 mt-1 font-body">Sets</p></div>}
                </div>
