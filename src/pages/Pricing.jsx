          ...
          {PLANS.map((plan, i) => (
            <motion.div key={plan.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
              className={`relative rounded-2xl p-6 flex flex-col transition-all ${
                plan.highlight
                  ? "bg-[#162232] border-2 border-[#B8960C]/60 shadow-lg shadow-[#B8960C]/10"
                  : "bg-[#162232] border border-[#2A3F58] hover:border-[#B8960C]/20"
              }`}>

              {/* Badge */}
              {plan.badge && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className={`text-xs font-body font-bold px-3 py-1 rounded-full ${plan.highlight ? "bg-[#B8960C] text-[#0A1628]" : "bg-[#2A3F58] text-[#F8F8F8]/60"}`}>
                    {plan.badge}
                  </span>
                </div>
              )}
              ...
              {/* Plan header */}
              <div className="mb-6">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${plan.highlight ? "bg-[#B8960C]/20 border border-[#B8960C]/30" : "bg-[#2A3F58] border border-[#2A3F58]"}`}>
              ...
              <div className="flex-1 space-y-3 mb-6">
                {plan.features.map(f => (
                  <div key={f} className="flex items-start gap-3">
                    <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${plan.highlight ? "bg-[#B8960C]/20" : "bg-[#2A3F58]"}`}>
                ...
                {plan.excluded.map(f => (
                  <div key={f} className="flex items-start gap-3 opacity-30">
                    <div className="w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 bg-[#2A3F58]">
              ...
              <Button
                ...
                className={`w-full h-12 rounded-xl font-semibold ${
                  currentPlan === plan.id
                    ? "bg-[#2A3F58] text-[#F8F8F8]/30 cursor-default"
                    : plan.highlight
                    ? "bg-[#B8960C] text-[#0A1628] hover:bg-[#D4AA10]"
                    : "border border-[#2A3F58] bg-transparent text-[#F8F8F8]/60 hover:border-[#B8960C]/30 hover:text-[#F8F8F8]"
                }`}>
                {loadingPlan === plan.id ? (
