...
function DashboardScreen() {
  ...
  return (
    <div className="bg-[#141414] border border-[#2A2A2A] rounded-2xl overflow-hidden w-full">
      {/* Dashboard header */}
      <div className="bg-[#0E0E0E] border-b border-[#2A2A2A] px-4 py-4">
        <p className="text-[#B8960C] text-[9px] font-body tracking-widest uppercase mb-0.5">Tennis · Intermediate</p>
        <p className="font-display text-xl text-[#F8F8F8]">Good morning.</p>
        <p className="text-[#F8F8F8]/30 text-[10px] font-body">Your body. Your season.</p>
      </div>
      {/* Stats row */}
      <div className="grid grid-cols-4 gap-2 p-3 border-b border-[#2A2A2A]">
        {[{ label: "This week", value: "3" }, { label: "Total time", value: "47m" }, { label: "Streak", value: "5" }, { label: "All time", value: "12" }].map(s => (
          <div key={s.label} className="bg-[#0E0E0E] border border-[#2A2A2A] rounded-xl p-2 text-center">
            <p className="font-mono text-sm text-[#B8960C]">{s.value}</p>
            <p className="text-[#F8F8F8]/25 text-[8px] font-body mt-0.5">{s.label}</p>
          </div>
        ))}
      </div>
      {/* Today recommendation */}
      <div className="mx-3 mt-3 mb-2 bg-[#0E0E0E] border border-[#B8960C]/20 rounded-xl px-3 py-2.5 flex items-center justify-between">
        <div>
          <p className="text-[#B8960C] text-[8px] font-body tracking-widest uppercase mb-0.5">Today's Recommendation</p>
          <p className="font-display text-sm text-[#F8F8F8]">Tennis Warmup</p>
          <p className="text-[#F8F8F8]/30 text-[9px] font-body">15 minutes</p>
        </div>
        <div className="bg-[#B8960C] rounded-lg px-2.5 py-1.5">
          <span className="text-[#080808] text-[10px] font-semibold font-body">Begin →</span>
        </div>
      </div>
      {/* Routines */}
      <div className="px-3 pb-3">
        <p className="text-[#B8960C] text-[8px] font-body tracking-widest uppercase mb-2">Your Routines</p>
        <div className="grid grid-cols-2 gap-1.5">
          {routines.map((r, i) => (
            <div key={i} className="bg-[#0E0E0E] border border-[#2A2A2A] rounded-xl p-2.5">
              <span className="text-[7px] font-body font-bold text-[#B8960C] bg-[#B8960C]/10 border border-[#B8960C]/20 px-1.5 py-0.5 rounded-full">{r.type}</span>
              <p className="font-display text-[11px] text-[#F8F8F8] mt-1.5 mb-0.5">{r.name}</p>
              <p className="text-[#F8F8F8]/25 text-[8px] font-body">{r.duration} · {r.exercises} exercises</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="relative z-10">
        ...
        <motion.nav initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center gap-3">
            <Link to="/coaches"><Button variant="ghost" className="text-[#F8F8F8]/50 hover:text-[#B8960C] text-sm">For Coaches</Button></Link>
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            <div>
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
                className="text-[#B8960C] text-xs font-body font-semibold tracking-[0.4em] uppercase mb-8">
                AI-Powered Prehab & Rehabilitation
              </motion.p>

              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                className="font-display text-6xl md:text-7xl text-[#F8F8F8] leading-[1.05] mb-8">
                Prevent Injuries.<br />
                <span className="text-[#B8960C]">Enhance Performance.</span>
              </motion.h1>

              <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                className="text-[#F8F8F8]/45 text-lg font-body leading-relaxed mb-10 max-w-md">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
                ...
                {[
                  ...
                  <div key={point} className="flex items-center gap-3.5">
                    ...
                    <span className="text-[#F8F8F8]/50 text-sm font-body tracking-wide">{point}</span>
                  </div>
                ))}
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
                ...
                <Link to="/login">
                  <Button size="lg" className="bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10] rounded-xl px-10 h-14 font-semibold text-base">
                    Start Free <ChevronRight className="w-4 h-4 ml-1" />
                  </Button>
                </Link>
                <Link to="/Pricing">
                  <Button size="lg" variant="outline" className="rounded-xl px-8 h-14 border-[#2A2A2A] text-[#F8F8F8]/30 hover:border-[#B8960C]/30 hover:text-[#F8F8F8]/60">
                    View Plans
                  </Button>
                </Link>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}
                className="flex flex-wrap gap-x-6 gap-y-1 border-t border-[#2A2A2A]/60 pt-6">
                {["Designed with physios", "15+ sports", "All levels", "Free to start"].map(t => (
                  <span key={t} className="text-[#F8F8F8]/20 text-xs font-body tracking-wide">{t}</span>
                ))}
              </motion.div>
            </div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4, duration: 0.8 }}
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }}
                ...
                <div className="bg-[#141414] border border-[#B8960C]/20 rounded-xl p-3 shadow-xl shadow-black/50">
                  <p className="text-[#B8960C] text-[8px] font-body tracking-widest uppercase mb-2">Pain Alert</p>
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-amber-400 rounded-full flex-shrink-0" />
                    <p className="text-[#F8F8F8]/60 text-[9px] font-body">Sarah M. — Pain 3/5 detected</p>
                  </div>
                  <div className="flex items-center gap-2 mt-1.5">
                    <div className="w-1.5 h-1.5 bg-emerald-400 rounded-full flex-shrink-0" />
                    <p className="text-[#F8F8F8]/60 text-[9px] font-body">James T. — 4 sessions this week</p>
                  </div>
                </div>
              </motion.div>
              {/* Glow behind */}
        ...
        <div className="max-w-6xl mx-auto px-5 mb-32">
          <div className="text-center mb-16">
            ...
            <h2 className="font-display text-5xl text-[#F8F8F8] leading-tight">Built for every part<br />of the athlete journey.</h2>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-5 mb-32">
          <div className="text-center mb-16">
            ...
            <h2 className="font-display text-5xl text-[#F8F8F8] leading-tight">From first session<br />to full recovery.</h2>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURES.map((f, i) => (
              <motion.div key={f.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }}
                className="bg-[#141414] border border-[#2A2A2A] rounded-2xl p-7 hover:border-[#B8960C]/20 transition-all group">
        ...
        <div className="max-w-6xl mx-auto px-5 mb-32">
          <div className="text-center mb-16">
            ...
            <h2 className="font-display text-5xl text-[#F8F8F8] leading-tight">Whoever you are.<br />Whatever your sport.</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {[
        ...
        <div className="max-w-6xl mx-auto px-5 mb-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            className="bg-[#141414] border border-[#2A2A2A] rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              <p className="kaya-label mb-2">Are you a coach or physio?</p>
              <h3 className="font-display text-2xl text-[#F8F8F8] mb-1">Manage your squad with Kaya.</h3>
              <p className="text-[#F8F8F8]/40 text-sm font-body">Monitor compliance, receive pain alerts and run AI team selection reports. 7-day free trial.</p>
            </div>
            <Link to="/coaches" className="flex-shrink-0">
              <Button className="bg-[#B8960C] text-[#080808] hover:bg-[#D4AA10] rounded-xl font-semibold h-12 px-8 whitespace-nowrap">
